...
City::City(int nRows, int nCols)
    : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0),gameHistory(m_rows, m_cols)
{
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    if (m_player == nullptr)
        return;
    bool didNotConvertAtLeastOneFlatulan = false;
    for (int k = 0; k < m_nFlatulans; )
    {
        ...
        {
            bool check = fp->possiblyGetConverted();
            if (check)
            {
                delete m_flatulans[k];
            ...
            {
                k++;
                didNotConvertAtLeastOneFlatulan = true;
            }
        }
        else
            k++;
    }
    if (didNotConvertAtLeastOneFlatulan)
        gameHistory.record(m_player->row(), m_player->col());
}
